...
public class UserCreateRequest {
    ...
    message = "email must be good")
    private String email;
    private String password;
    //@Pattern(regexp = "\\+\\d{1,2}\\s?(\\d{1,3})\\s?\\d{3}-\\d{4}", message = "Please enter valid phone number")
    private String phone;
    // такой регекс для международных имен например русских
    //@Pattern(regexp = "/^[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]+$/u", message = "Enter valid name")
